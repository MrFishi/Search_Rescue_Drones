...
**Detector comparison, answers O2.** Train each detector candidate (YOLO variants, Mamba-hybrid) on the same combined dataset, eval on the same held-out test set, and benchmark accuracy, latency, and resource usage together on the actual Orin Nano Super. A model that wins on mAP but can't hit real-time frame rate on-device isn't a real answer to O2, so all three numbers need to be reported side by side. The top two architectures (by mAP, among those that clear the on-device latency and memory budget) are both carried into the A0/A2 head comparison, not just the single winner — this is a deliberate, bounded relaxation of strict elimination that catches the one interaction the funnel is most likely to miss, a runner-up that only becomes best once paired with A2's separate HPI head, for about two extra runs rather than the ~120 a full grid would cost. From the head comparison down, a single architecture-plus-head winner is carried into everything below, so I'm still not multiplying every later experiment across every architecture.
